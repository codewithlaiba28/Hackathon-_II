...
get_feature_paths() {
    local repo_root_win=$(get_repo_root) # Get Windows path first
    local repo_root="$repo_root_win"
    ...
    # If SPECIFY_FEATURE is explicitly set, use it directly
    if [[ -n "${SPECIFY_FEATURE:-}" ]]; then
        feature_dir="$repo_root/specs/$SPECIFY_FEATURE"
    else
        # Otherwise, use prefix-based lookup to support multiple branches per spec
