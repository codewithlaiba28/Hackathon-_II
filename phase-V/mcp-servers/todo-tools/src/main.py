...
# TaskTag model for hierarchical tagging
...
# Log DB URL (masking password)
masked_db_url = db_url
if ":" in db_url and "@" in db_url:
    try:
        parts = db_url.split("@")
        prefix = parts[0].split(":")
        masked_db_url = f"{prefix[0]}:***@{parts[1]}"
    except: pass
logger.info(f"Using DB URL: {masked_db_url}")
engine = create_engine(db_url)

@mcp.tool(description="Create a new task with optional priority, due date, tags, and recurrence settings")
def add_task(
    user_id: str, 
    title: str, 
    ...
    Args:
        ...
        tags: Comma-separated tags like 'study,work' (optional)
    ...
    """
    logger.info(f"add_task tool called for user {user_id}: {title} (priority={priority}, due_date={due_date}, tags={tags})")
    try:
        with Session(engine) as session:
            ...
            if tags:
                ...
                for tag_name in tag_list:
                    task_tag = TaskTag(task_id=task.id, tag=tag_name)
                    session.add(task_tag)
                    added_tags.append(tag_name)
                session.commit()
                logger.info(f"Added {len(added_tags)} tags to task {task.id}")
